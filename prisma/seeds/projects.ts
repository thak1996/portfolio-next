import { prisma } from '../../src/lib/db';

export async function seedProjects() {
  const projects = [
    {
      title: '(iOS & Android)',
      description: 'Aplicativo completo (Flutter + Laravel). Ciclo de vida completo até publicação na App Store.',
      imageUrl: 'https://lh3.googleusercontent.com/aida-public/AB6AXuBd5GAnzWysO9w8UiXGyVdCixnVKQe1sX6xfUFlF-6VKiRaYar5ZKJwBmS05p0Tj1GtMM0sKAs65BtGbL7701ubKxl7nK8ijplYgEWnLMXouTHXGXOPSbSulbSWDLVkzB575zxjPIVt6Fqwb42QiMTymky-AtJAvFD91nJHwDG0Zy59o9ublA_zqWTr7Zuilb5oUBhZWZ90TZjcAhxtff1nnyZjYHoolYh8CM4rqlqetq9yJFrawFxlEPodkGbIrhdLFi1GH9wITJk',
      tags: ['Flutter', 'Laravel'],
      detailsLink: '#',
      featured: true,
    },
    {
      title: 'Dashboard de Gestão',
      description: 'Dashboard de gestão / Sistema de leads (React, Node, PostgreSQL). Foco em usabilidade e segurança.',
      imageUrl: 'https://lh3.googleusercontent.com/aida-public/AB6AXuBH9cPQUXn4IFvr6TGI_FT5jHaOiJpR06c3TX8h7n7_nnLSZynILbBirB-Uj_KO0OZyrTgpvjkkrc0mMttkdXsmS7YbhosziZHqhCx63kHg4Kd_7hh_-ieWlS6a45Gjf6BCNYFiCPdBJf2NdPQ0WavQq2OWosBUUhrkfGBnn_2H7KBR1oZS8CtR7Jsipw1hxa6qNJ5cvWfF9vpxcrXduHs1r3ftldKg7xS7O1YCwKvN5jWy74g40a3nZU4-U0NNOYXxxQ8rPvCnKmk',
      tags: ['React', 'Node'],
      detailsLink: '#',
      featured: true,
    },
    {
      title: 'FDS Logística Website',
      description: 'Sistema de gerenciamento de mudanças e logística com painel administrativo e dashboards de controle.',
      imageUrl: 'https://via.placeholder.com/800x600/171f33/00f0ff?text=FDS+Logistica',
      tags: ['PHP Laravel', 'VPS', 'Domínios', 'UI/UX'],
      detailsLink: 'https://www.fdslogistica.com.br/',
      featured: true,
    },
    {
      title: 'Brincar e Conectar App',
      description: 'Aplicação mobile para instituições e orfanatos, focada em conexão social.',
      imageUrl: 'https://via.placeholder.com/800x600/171f33/4edea3?text=Brincar+e+Conectar',
      tags: ['Flutter', 'UI/UX', 'Dart', 'Bloc'],
      githubLink: 'https://github.com/thak1996/brincar_e_conectar_flutter',
      detailsLink: 'https://github.com/thak1996/brincar_e_conectar_flutter',
      featured: true,
    },
    {
      title: 'RH Manager',
      description: 'Gerenciador de RH construído com Laravel 10, Docker e MySQL.',
      imageUrl: 'https://via.placeholder.com/800x600/171f33/dae2fd?text=RH+Manager',
      tags: ['UI/UX', 'PHP Laravel', 'MySQL', 'Docker'],
      githubLink: 'https://github.com/thak1996/rh-manager',
      detailsLink: 'https://github.com/thak1996/rh-manager',
      featured: true,
    }
  ];

  for (const proj of projects) {
    await prisma.project.create({
      data: proj
    });
    console.log(`Projeto criado: ${proj.title}`);
  }
}
